...
import { useLoaderData, useNavigate, Link, useFetcher, useRouteLoaderData } from 'react-router';
...
const DEFAULT_INVENTORY_SETTINGS = {
  ...
  outOfStockBadgeColor: "red",
  criticalStockThreshold: 5,
  criticalStockMessage: "Only few left!",
  criticalStockBadgeColor: "orange",
  lowStockThreshold: 10,
  lowStockMessage: "Few left",
  lowStockBadgeColor: "yellow"
};
...
export const meta = ({ data }) => {
  return [{ title: data?.term ? `Search for "${data.term}"` : 'Search Products | Hydrogen' }];
};
...
/**
 * Regular search - fetches all products or search results
 */
async function regularSearch({ request, context }) {
  ...
  // If no search term, fetch ALL products with pagination
  if (!term) {
    while (hasNextPage) {
    ...
    const { search } = await storefront.query(SEARCH_API_QUERY, {
      variables: {
        first: 250,
        term: term,
    ...
    allProducts = search.nodes.filter(node => node.__typename === 'Product');
    totalProducts = allProducts.length;
  }
  ...
  if (safeSettings.enabled && isLoggedIn && accessToken) {
    try {
      ...
      if (customerId) {
        const adminQuery = `
          query getCustomerWishlistInSearch($id: ID!) {
            customer(id: $id) {
              id
  ...
  // Fetch quick view configuration with fallback
  let quickViewConfig;
  try {
...
async function predictiveSearch({ request, context }) {
  ...
  const activeCountry = context.storefront?.i18n?.country?.toLowerCase() || 'us';
  const activeCurrency = items?.products?.[0]?.selectedOrFirstAvailableVariant?.price?.currencyCode || 'USD';
  const locale = { country: activeCountry, currency: activeCurrency };

  return { type: 'predictive', term, result: { items, total }, locale };
}
...
export default function SearchPage() {
  ...
  const getHeadingStyle = (level = 'h1') => {
    if (!globalData?.headingSizes) {
      ...
      return {
        fontSize: `${defaultSizes[level] || 32}px`,
    ...
    return {
      fontSize: `${sizeMap[level] || sizes.h1 || 32}px`,
  ...
  const handleGlobalSearchChange = (newSearchTerm) => {
    setSearchTerm(newSearchTerm);
    const countryPrefix = locale?.country && locale.country !== 'us' ? `/${locale.country}` : '';
    if (newSearchTerm.trim()) {
      navigate(`${countryPrefix}/search?q=${encodeURIComponent(newSearchTerm)}`, { replace: true });
    } else {
      navigate(`${countryPrefix}/search`, { replace: true });
    }
  };
  ...
  const applyFilters = () => {
    ...
    if (selectedFilters.vendors?.length > 0) {
      filtered = filtered.filter(product => selectedFilters.vendors.includes(product.vendor));
    }

    if (selectedFilters.productTypes?.length > 0) {
      filtered = filtered.filter(product => selectedFilters.productTypes.includes(product.productType));
    }

    if (selectedFilters.colors?.length > 0) {
      filtered = filtered.filter(product => {
        const colorOption = product.options?.find(opt => opt.name.toLowerCase().includes('color'));
        return colorOption?.values?.some(color => selectedFilters.colors.includes(color));
      });
    }
    ...
    if (selectedFilters.sizes?.length > 0) {
      filtered = filtered.filter(product => {
        ...
        return sizeOption?.values?.some(size => selectedFilters.sizes.includes(size));
      });
    }

    if (selectedFilters.tags?.length > 0) {
      filtered = filtered.filter(product => product.tags?.some(tag => selectedFilters.tags.includes(tag)));
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center px-[7%] mb-8 w-full">
        ...
        >
         {term ? `Search for "${term}"` : "Search"}
        </h1>
        <nav className="flex items-center text-sm">
          <Link 
            to="/" 
            className="font-bold text-[14px] leading-[24px] tracking-[0.2px] text-center transition-colors"
            onMouseEnter={(e) => {
      ...
      <div className="flex flex-col px-[7%] lg:flex-row gap-8">
        {/* Filter Section - Pass global data with fallbacks */}
        <Filter
...
>
      ...
      />
          ...
          {/* Product Grid - Updated to pass correct props */}
            ...
            <div className="text-center py-12">
                ...
                <div className="max-w-2xl mx-auto mt-6">
                  <div className="flex flex-wrap items-center gap-3 justify-center">
                    ...
                    {suggestions.map((suggestion, index) => (
                      <Link
                        key={index}
                        to={`${countryPrefix}/search?q=${encodeURIComponent(suggestion)}`}
                        onClick={() => setSearchTerm(suggestion)}
                        className="px-4 py-2 text-sm bg-gray-100 border border-gray-300 rounded-md transition"
                        style={getButtonStyle('secondary')}
                        onMouseEnter={(e) => {
                          e.currentTarget.style.backgroundColor = formatColor(globalData?.buttons?.secondaryHoverBg) || '#D1D5DB';
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.backgroundColor = formatColor(globalData?.buttons?.secondaryBg) || '#E5E7EB';
                        }}
                      >
                        {suggestion}
                      </Link>
              ...
              <div className="flex justify-center mt-8">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-md transition font-medium"
  ...
  borderRadius: '0px'
      ...
      <Analytics.SearchView data={{ searchTerm: term, searchResults: { total: filteredProducts.length } }} />
    </div>
  );
}
...
function ProductItem({
  ...
  return (
    ...
    >
      <Link to={`${countryPrefix}/products/${product.handle}`} className="block" prefetch="intent">
          ...
          {image && (
            <div className="relative overflow-hidden aspect-square">
              <img
                ...
                className={`w-full h-full object-cover transition-transform duration-700 ${isHovered ? 'scale-110' : 'scale-100'} ${loaded ? 'opacity-100' : 'opacity-0'}`}
              ...
              <div className={`absolute inset-0 bg-black/40 flex items-end justify-center pb-4 gap-2 transition-all duration-300 ${isHovered && !showVariantSelector ? 'opacity-100' : 'opacity-0'}`}>
                {isWishlistEnabled && (
                  <button
                    ...
                    className={`bg-white w-10 h-10 flex items-center justify-center transition-all duration-300 transform hover:scale-110 shadow-lg ${isWished ? "text-red-500" : "text-black"} hover:bg-black ${wishlistLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
...
const SEARCH_API_QUERY = `#graphql
